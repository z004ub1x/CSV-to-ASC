...
def _read_file_rows(filepath):
    ...
    Open a CSV or Excel file and return:
        (header, row_iter, total_data_rows)

    header         – list[str]  : column names from row 1
    row_iter       – iterator   : yields each data row as list[str]
                                  (header row is already consumed)
    total_data_rows – int       : number of data rows (excluding header)

    Returns (None, None, 0) on any error.

    IMPORTANT: For CSV the iterator reads the file lazily; for Excel the entire
    workbook is loaded into memory by openpyxl (read_only=True keeps RAM low).
    """
    ftype = _get_file_type(filepath)

    if ftype == 'csv':
        try:
            # We need two passes: one to count rows, one to iterate.
            # Count first (fast).
        ...
        try:
            ...
            # Count data rows (we need total for progress bar).
            # read_only worksheets expose ws.max_row (may be None for some files).
            max_row = ws.max_row
            if max_row is not None and max_row > 1:
                total = max_row - 1
            else:
                # Fallback: consume and re-open (rare edge case).
                total = sum(1 for _ in rows_iter)
                wb.close()
                wb = openpyxl.load_workbook(filepath, read_only=True, data_only=True)
                ws = wb.active
                rows_iter = ws.iter_rows(values_only=True)
                next(rows_iter, None)   # skip header again
            ...
            return header, _xlsx_iter(), total
...
def estimate_sample_rate_from_timestamps(input_filepath, ts_type, n_samples=_TS_SAMPLE_ROWS):
    ...
    timestamps_seconds = []
    try:
        header, row_iter, _ = _read_file_rows(input_filepath)
        if row_iter is None:
            return None, None, 0, None
        for row in row_iter:
            if not row:
...
def detect_absolute_timestamp(input_filepath):
    """
    Read the first two rows of a CSV or Excel file and detect the timestamp type.
...
def select_batch_files(root):
    """
    Show a multi-select file dialog so the user can pick one or more CSV or
    Excel files in a single operation.  Returns a list of absolute file paths
    (may be empty).
    """
    ...
    # Build filetypes list dynamically so Excel option only appears when available
    ...
    paths = filedialog.askopenfilenames(
        title='Select CSV / Excel File(s) to Convert  (hold Ctrl / Shift for multiple)',
...
def _resolve_file_metadata(root, input_filepath):
    ...
    """
    # ── Read header ──────────────────────────────────────────────────────────
    header, _row_iter_unused, _ = _read_file_rows(input_filepath)
    if header is None:
        messagebox.showerror('Error',
            f"'{os.path.basename(input_filepath)}' is empty, unreadable, or has no header.",
            parent=root)
        return None
    ...
    if has_timestamp_col:
        ...
        # ── FIX (Issue 1): build sr_line with a proper if/else, not a ternary ──
        if suggested_hz is not None:
            sr_line = (
...
def create_simcenter_asc_file(input_filepath, output_filepath, header_attributes,
    ...
    try:
        ...
        # ── Get row iterator and total count via unified reader ──────────────
        _hdr, row_iter, total_lines = _read_file_rows(input_filepath)
        if row_iter is None:
            messagebox.showerror('Error',
                f"Could not read '{os.path.basename(input_filepath)}'.", parent=root)
            return False, 0, ''
...
def main_conversion_process_gui():
    ...
    while True:
        # ── Step 1: select one or more CSV / Excel files ───────────────────
