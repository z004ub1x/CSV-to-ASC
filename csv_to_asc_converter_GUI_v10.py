...
# ── Optional Excel support (requires openpyxl) ───────────────────────────────
try:
    import openpyxl
...
# ── Unit translation table (Excel → Simcenter) ───────────────────────────────
_XLSX_UNIT_MAP = {
    '(percent)': '%',
    '(n/a)':     '/',
}

def _translate_xlsx_unit(raw_unit):
    """Translate Excel-style unit strings to Simcenter equivalents."""
    if raw_unit is None:
        return ''
    cleaned = str(raw_unit).strip()
    return _XLSX_UNIT_MAP.get(cleaned.lower(), cleaned)
...
def _get_file_type(filepath):
    """Return 'csv', 'xlsx', or None based on file extension."""
...
def _parse_xlsx_structure(filepath):
    """
    Parse an Excel file with the following layout:
        Rows starting with '#'  -> comment/metadata rows  (skip)
        First non-comment row   -> channel names           (CHANNELNAME)
        Second non-comment row  -> descriptions            (ignored)
        Third non-comment row   -> units                   (UNIT, translated)
        Remaining rows          -> data

    Returns:
        (channel_names, units, data_rows, total_data_rows)
        Returns (None, None, None, 0) on error.
    """
    if not _OPENPYXL_AVAILABLE:
        messagebox.showerror(
            ...
            'CSV files can still be converted without it.')
    ...
    # Find the first non-comment row
    header_start = None
    ...
    if header_start is None:
        messagebox.showerror('Excel Parse Error',
            f"'{os.path.basename(filepath)}' contains only comment rows — no data found.")
        return None, None, None, 0

    # We need at least 3 non-comment rows (names, descriptions, units) + 1 data row
    non_comment = all_rows[header_start:]
    if len(non_comment) < 4:
        messagebox.showerror('Excel Parse Error',
            f"'{os.path.basename(filepath)}' does not have enough header rows "
            f"(expected: channel names, descriptions, units, then data).")
        return None, None, None, 0

    name_row  = non_comment[0]   # channel names
    # non_comment[1] = descriptions — intentionally ignored
    unit_row  = non_comment[2]   # units
    data_rows = non_comment[3:]  # actual data

    # Convert to clean string lists
    n_cols = max(len(name_row), len(unit_row))
    channel_names = []
    units         = []
    for col_i in range(n_cols):
        raw_name = name_row[col_i] if col_i < len(name_row) else None
        raw_unit = unit_row[col_i] if col_i < len(unit_row) else None
        channel_names.append(str(raw_name).strip() if raw_name is not None else '')
        units.append(_translate_xlsx_unit(raw_unit))

    # Convert data rows to list[list[str]]
    str_data_rows = []
    for row in data_rows:
        str_data_rows.append([str(c) if c is not None else '' for c in row])

    return channel_names, units, str_data_rows, len(str_data_rows)

# ── Unified row reader ────────────────────────────────────────────────────────
def _read_file_rows(filepath):
    """
    Open a CSV or Excel file and return:
        (header, row_iter, total_data_rows, pre_filled_units)

    header            - list[str]  : column names from row 1
    row_iter          - iterator   : yields each data row as list[str]
    total_data_rows   - int        : number of data rows (excluding header)
    pre_filled_units  - list[str] or None
                        For Excel: translated unit strings (one per column).
                        For CSV:   None (no pre-filled units).

    For Excel files the data iterator performs sample-and-hold fill:
    any empty cell in a data row is replaced with the most recent non-empty
    value seen in that column (or '' if no prior value exists).
    """
    ...
    if ftype == 'csv':
        try:
            ...
            fh     = open(filepath, 'r', newline='', encoding='utf-8')
            reader = csv.reader(fh)
            header_raw = next(reader, None)
            if not header_raw:
                fh.close()
                return None, None, 0, None
            header = [str(c).strip() for c in header_raw]
            ...
            return header, _csv_iter(), total, None

        except Exception:
            return None, None, 0, None

    elif ftype == 'xlsx':
        channel_names, units, data_rows, total = _parse_xlsx_structure(filepath)
        if channel_names is None:
            return None, None, 0, None

        # Sample-and-hold iterator
        def _xlsx_iter():
        ...
        return channel_names, _xlsx_iter(), total, units
...
def estimate_sample_rate_from_timestamps(input_filepath, ts_type,
                                          n_samples=_TS_SAMPLE_ROWS):
    if ts_type not in ('DDD', 'SECONDS'):
        return None, None, 0, None
    timestamps_seconds = []
    try:
        header, row_iter, _, _units = _read_file_rows(input_filepath)
        if row_iter is None:
            return None, None, 0, None
        for row in row_iter:
            if not row:
...
def _ask_recording_year(root, context_label, default_year=None):
    ...
    year_entry = ttk.Entry(dialog, textvariable=year_var, width=8,
                           font=('Arial', 11))
    ...
    ttk.Label(quick_frame, text='Quick select:',
    ...
    def _on_cancel():
        # NOTE: Cancel intentionally returns the default year (current year).
        # This is by design (v6 spec): 'Cancel keeps the default'.
        # result['year'] is already set to default_year — no update needed.
        dialog.destroy()
...
def detect_absolute_timestamp(input_filepath):
    """
    Read the first data row of a CSV or Excel file and detect the timestamp type.
    Returns (ts_type, first_col_value, first_col_name) or (None, None, None).
    """
    try:
        header, row_iter, _, _units = _read_file_rows(input_filepath)
        if header is None or row_iter is None:
            return None, None, None
        first_col_name  = header[0] if header else ''
        first_data_row  = next(row_iter, None)
        if not first_data_row:
            return None, None, None
        first_col_value = str(first_data_row[0]).strip()
...
def _ask_recording_date(root, first_ts_value):
    ...
    ttk.Button(nav_frame, text='\u2039  Prev',
               command=_prev_month, width=8).pack(side='left', padx=4)
    ttk.Label(nav_frame, textvariable=month_label_var,
              font=('Arial', 11, 'bold'), width=16,
              anchor='center').pack(side='left', padx=8)
    ttk.Button(nav_frame, text='Next  \u203a',
               command=_next_month, width=8).pack(side='left', padx=4)
    ...
    ttk.Label(sel_frame, textvariable=selected_date_str,
              font=('Arial', 9, 'bold'),
              foreground='#0055aa').pack(side='left', padx=6)

    def _jump_to_today():
        view_year.set(today.year)
        view_month.set(today.month)
        _select_day(today.day)

    def _on_ok():
        if _chosen['date'] is None:
            messagebox.showwarning(
                'No Date Selected',
                'Please click a day on the calendar, or click Skip '
                'to use a placeholder date.',
                parent=dialog)
            return
    ...
    ttk.Button(btn_frame, text='\u2605 Today',
               command=_jump_to_today).pack(side='left', padx=6)
    ttk.Button(btn_frame, text='OK',
               command=_on_ok).pack(side='left', padx=6)
    ttk.Button(btn_frame, text='Skip',
               command=_on_skip).pack(side='left', padx=6)
...
def get_channel_metadata_and_sampling_gui(original_header,
                                           has_timestamp_col=False,
                                           suggested_hz=None,
                                           suggestion_confidence=None,
                                           suggestion_delta_s=None,
                                           suggestion_n_deltas=None,
                                           pre_filled_units=None):
    """
    pre_filled_units: list[str] or None.
      If provided (Excel path), the UNIT column is pre-populated from the
      file's own unit row.  The user can still edit any value before clicking OK.
    """
    ...
    num_columns = len(original_header)
    ...
    if pre_filled_units is not None:
        info_text += '  \u2139 Units pre-filled from Excel file header.'
    ...
    for i, col_name in enumerate(original_header):
        ...
        if is_ts_row:
            ...
            # Use pre-filled unit if available, otherwise default to 'g'
            default_unit = 'g'
            if pre_filled_units is not None and i < len(pre_filled_units):
                pu = pre_filled_units[i]
                if pu:
                    default_unit = pu
            u_entry.insert(0, default_unit)
            data_col_index += 1
    ...
    if suggested_hz is not None:
        confidence_colours = {'HIGH':   '#1a7a1a',
                               'MEDIUM': '#8a6000',
                               'LOW':    '#aa2200'}
        confidence_icons   = {
        ...
        banner   = (f'Auto-detected from {suggestion_n_deltas} inter-sample deltas:  '
                    f'{suggested_hz:,.4f} Hz  |  '
                    f'\u0394t\u0305 = {suggestion_delta_s:.9f} s  |  {icon_txt}')
        ttk.Label(sampling_frame, text=banner,
                  font=('Arial', 8, 'italic'),
    ...
    ttk.Label(sampling_frame,
              text='You may edit this value freely. DELTA updates live.',
              font=('Arial', 8)).grid(
        row=2, column=0, columnspan=3, padx=5, sticky='w')
    ...
    def _update_delta(*args):
        try:
            ...
            delta_var.set(f'{1.0/sf:.9f} seconds' if sf > 0
                          else 'Invalid (must be > 0)')
        except ValueError:
    ...
    def on_ok():
        ...
        for ch_e, u_e, id_e, is_ts in entry_widgets:
            ...
            if not ch_val:
                messagebox.showerror('Input Error',
                    'CHANNELNAME cannot be empty.', parent=dialog)
                return
            if not u_val:
                messagebox.showerror('Input Error',
                    f"UNIT for '{ch_val}' cannot be empty.", parent=dialog)
                return
            if not id_val or not id_val.isdigit():
                messagebox.showerror('Input Error',
                    f"EDA_CHANNELId for '{ch_val}' must be a non-empty number.",
                    parent=dialog)
                return
        ...
        if not sf_str:
            messagebox.showerror('Input Error',
                'Sampling Frequency cannot be empty.', parent=dialog)
            return
        try:
            sf = float(sf_str)
            if sf <= 0:
                messagebox.showerror('Input Error',
                    'Sampling Frequency must be positive.', parent=dialog)
                return
        except ValueError:
            messagebox.showerror('Input Error',
                'Invalid Sampling Frequency.', parent=dialog)
            return
    ...
    if result['cancelled']:
        return None
    return {'UNIT':          result['units'],
            'CHANNELNAME':   result['channel_names'],
...
def select_batch_files(root):
    ...
    if _OPENPYXL_AVAILABLE:
        filetypes.insert(0, ('CSV and Excel files', '*.csv *.xlsx *.xlsm'))
        filetypes.append(('Excel files', '*.xlsx *.xlsm'))
    filetypes.append(('All files', '*.*'))

    paths = filedialog.askopenfilenames(
        title='Select CSV / Excel File(s) to Convert  '
              '(hold Ctrl / Shift for multiple)',
...
def _resolve_file_metadata(root, input_filepath):
    """
    For a single CSV or Excel file:
      1. Read header (and pre-filled units for Excel).
      2. Detect timestamp type and first value.
      3. Estimate sample rate.
      4. Show timestamp info messagebox.
      5. Ask for recording date / year as appropriate.
      6. Show channel metadata + sampling frequency GUI
         (units pre-populated for Excel files).

    Returns (simcenter_header_attrs, original_header, sampling_freq, ts_info)
    or None on cancellation.
    """
    header, _iter_unused, _, pre_filled_units = _read_file_rows(input_filepath)
    if header is None:
        messagebox.showerror('Error',
            f"'{os.path.basename(input_filepath)}' is empty, "
            f"unreadable, or has no header.",
            parent=root)
        return None
    ...
    if has_timestamp_col:
        ...
        if suggested_hz is not None:
            sr_line = (
                f'\n\nSample rate estimated from {suggestion_n} '
                f'inter-sample deltas:\n'
                f'  \u0394t\u0305 = {suggestion_delta:.9f} s\n'
                f'  \u2192 {suggested_hz:,.4f} Hz  ({suggestion_conf} confidence)\n\n'
                'The sampling frequency field has been pre-filled '
                '\u2014 please verify.'
            )
        else:
    ...
    combined = get_channel_metadata_and_sampling_gui(
        ...
        pre_filled_units=pre_filled_units)

    if combined is None:
        return None

    sampling_freq = combined.pop('SAMPLING_FREQ')
    ts_info = {'ts_type':        ts_type,
               'first_ts_value': first_ts_value,
               'ts_col_name':    ts_col_name,
               'recording_date': recording_date}
    return combined, original_header, sampling_freq, ts_info
...
def create_simcenter_asc_file(input_filepath, output_filepath, header_attributes,
    ...
    try:
        ...
        _hdr, row_iter, total_lines, _units = _read_file_rows(input_filepath)
        if row_iter is None:
            messagebox.showerror('Error',
                f"Could not read '{os.path.basename(input_filepath)}'.",
                parent=root)
            return False, 0, ''
        ...
        simcenter_header = '\n'.join(header_lines)
        update_interval  = max(1, total_lines // 100) if total_lines > 0 else 1

        with open(output_filepath, 'w', newline='', encoding='utf-8') as outfile:
            outfile.write(simcenter_header)
            rows_written = 0
            for row in row_iter:
                data_row = (row[1:] if ts_type in ('DDD', 'SECONDS')
                            and len(row) > 1 else row)
                outfile.write(','.join(str(v) for v in data_row) + '\n')
                rows_written += 1
                if (rows_written % update_interval == 0
                        or rows_written == total_lines):
                    pct = ((rows_written / total_lines * 100)
        ...
        messagebox.showerror('Error',
            ...
            f"'{os.path.basename(input_filepath)}': {e}",
            parent=root)
        return False, 0, ''
...
def _show_batch_summary(root, results):
    ...
    for i, r in enumerate(results, 1):
        ...
        if r['ok']:
            ...
            text.insert('end',
                f"       Conversion failed \u2014 see earlier error dialog.\n\n")

    text.configure(state='disabled')
    ttk.Button(dialog, text='Close', command=dialog.destroy).pack(pady=10)
    root.wait_window(dialog)

# ── Session date/year cache reset ─────────────────────────────────────────────
...
def main_conversion_process_gui():
    ...
    while True:
        ...
        for idx, input_csv_file in enumerate(input_files, 1):
            ...
            if meta is None:
                choice = messagebox.askyesno(
                    ...
                    f"Skip this file and continue with the remaining "
                    f"{total_files - idx}?",
                    parent=root)
                if not choice:
                    break
            ...
            simcenter_header_attrs, original_header, sampling_freq, ts_info = meta
            ...
            output_asc_file = filedialog.asksaveasfilename(
                ...
                filetypes=[('Simcenter Testlab ASC files', '*.asc'),
                           ('All files', '*.*')],
                parent=root)
        ...
        batch_results = []
        for job_idx, (in_path, out_path, attrs, sf, ts_info) in \
                enumerate(file_jobs, 1):
