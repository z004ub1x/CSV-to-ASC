#!/usr/bin/env python3
"""
csv_to_asc_converter_GUI_v13.py
Converts CSV / XLSX files to Simcenter Testlab ASC format.

Changes vs v12
==============
FIX 8 - Restored v8 header list-literal format
    UNIT, CHANNELNAME and EDA_CHANNELId are written as Python
    list literals (e.g. ['g', 'g', 'g']) to match v8 exactly.
    EDA_AbsoluteTime is wrapped in [...] as in v8, e.g.:
        EDA_AbsoluteTime = ['2026-04-01 13:10:00 ms 0']

FIX 9 - Suggested sample rate rounded to nearest integer
    The raw float Hz estimate (e.g. 7500.19) is rounded to the
    nearest integer (e.g. 7500) immediately after estimation.
    The rounded value is shown in the hint label and pre-filled
    in the sampling-frequency entry box.
"""
...
# ---------------------------------------------------------------------------
# Sample-rate estimation  -- FIX 9: round to nearest integer
# ---------------------------------------------------------------------------
...
def estimate_sample_rate(filepath, ts_type):
    """Return (hz_int, conf, delta_s, n_deltas) or (None,None,None,None).
    hz_int is the raw float Hz rounded to the nearest integer.
    """
    try:
        timestamps = []
        if _is_xlsx(filepath):
        ...
        hz_raw    = 1.0 / avg_delta
        hz_int    = int(round(hz_raw))          # FIX 9: round to nearest integer
        spread    = max(deltas) - min(deltas)
        conf = ('HIGH'   if spread < avg_delta * 0.01 else
                'MEDIUM' if spread < avg_delta * 0.10 else 'LOW')
        return hz_int, conf, avg_delta, len(deltas)
    except Exception:
        return None, None, None, None
...
def get_channel_metadata_and_sampling_gui(original_csv_header,
    ...
    if suggested_hz is not None:
        # FIX 9: suggested_hz is already an integer; show it clearly
        hint = (f'Suggested: {suggested_hz} Hz  '
    ...
    if suggested_hz is not None:
        sf_entry.insert(0, str(suggested_hz))   # integer, no decimal point
...
def _resolve_file_metadata(root, input_filepath):
    ...
    if has_timestamp_col:
        ...
        if suggested_hz:
            msg += (f"\nEstimated sample rate: {suggested_hz} Hz "
...
# ---------------------------------------------------------------------------
# ASC header builder -- FIX 8: v8 list-literal format
# ---------------------------------------------------------------------------

def _build_eda_abs_time_str(ts_type, first_ts_value, recording_date):
    """Return the inner quoted string, e.g. '2026-04-01 13:10:00 ms 0'
    The caller wraps it in [...] to produce the full EDA_AbsoluteTime line.
    ms value trailing zeros are stripped, matching v8 behaviour.
    """
    try:
        if ts_type == 'DDD':
            ...
            return f"'{date_pfx}{hh:02d}:{mm:02d}:{ss_int:02d} ms {ms_str}'"
        elif ts_type == 'SECONDS':
...
def _build_asc_header(header_attributes, sampling_freq, ts_info):
    """Build ONE BEGIN...END block.
    UNIT / CHANNELNAME / EDA_CHANNELId use Python list-literal format (FIX 8).
    EDA_AbsoluteTime is wrapped in [...] (FIX 8).
    """
    ...
    # FIX 8: format as Python list literals to match v8 exactly
    unit_str = repr(header_attributes['UNIT'])   # e.g. ['g', 'g', 'g']
    name_str = repr(header_attributes['CHANNELNAME'])
    id_str   = repr(header_attributes['EDA_CHANNELId'])

    eda_abs_time_line = None
    if ts_type and first_ts_value:
        inner = _build_eda_abs_time_str(ts_type, first_ts_value, recording_date)
        if inner:
            eda_abs_time_line = f'EDA_AbsoluteTime = [{inner}]'
...
# ---------------------------------------------------------------------------
# Data writer -- XLSX sample-and-hold (FIX 5 from v12)
# ---------------------------------------------------------------------------
